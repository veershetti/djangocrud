...
conn = mcdb.connect(host="localhost", user="root", passwd="143", database='db_ecom_stock')
...
def categoryaddprocess(request):
    if request.method == 'POST':
        ...
        isdeleted='0'
        sql="INSERT INTO `tb_category`(`category_name`,`is_deleted`) VALUES ('{}','{}')".format(catname,isdeleted)
        cur.execute(sql)
        print(sql)
        conn.commit()
